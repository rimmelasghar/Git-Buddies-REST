...
from schema import  User,Token,TokenData,UserInDB,UserIn,Repo
from models import loginTable,repoTable
...
async def create_boosted_repository(repo : Repo,current_user: User = Security(get_current_active_user, scopes=["user"])):
    user = session.query(loginTable).filter(loginTable.username == user.username).first()
    repo = repoTable(user_id=user.id,title=repo.title,status=repo.status,repolink=repo.repolink)
    session.add(repo)
    session.commit()
